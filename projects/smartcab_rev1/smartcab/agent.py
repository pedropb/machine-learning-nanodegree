...
class LearningAgent(Agent):
    ...
    def choose_action(self, state):
        ...
        if not self.learning:
            ...
            n = random.random()
            if self.epsilon > n:
                print "Agent decided to explore {:.4f} < {:.4f}".format(n, self.epsilon)
                action = self.valid_actions[random.randrange(0,len(self.valid_actions))]
            else:
                print "Agent selected maxQ {:.4f} >= {:.4f}".format(n, self.epsilon)
    ...
    def update(self):
        ...
        print "Agent.update() - state: {}, Q[state]-before: {}, action: {}, reward: {}, Q[state]-after: {}".format(state, s, action, reward, self.Q[state])
